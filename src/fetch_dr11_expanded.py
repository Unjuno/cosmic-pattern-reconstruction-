...
def angular_sep(ra1, dec1, ra2, dec2):
    r1, r2 = np.deg2rad([ra1, ra2])
    d1, d2 = np.deg2rad([dec1, dec2])
...
def query(sql: str, attempts: int = 4) -> str:
    """Return Data Lab CSV text.

    Header-only CSV is a valid zero-row query result and must not be treated as
    a transport failure. Explicit service error strings are retried instead.
    """
    last = None
    for i in range(attempts):
        try:
            ...
            if stripped.lower().startswith(('error:', 'error\n', '<html', '<!doctype')):
                raise RuntimeError(f'service error response: {stripped[:240]}')
            if not stripped:
                raise RuntimeError('empty response without CSV header')
            return out
...
def candidate_centers():
    cand = [
        (float(ra), float(dec))
        for dec in [-55, -45, -35, -25, -15, -5, 5, 15, 25]
        for ra in np.arange(0, 360, 12)
    ]
...
def fetch_candidate(ra0, dec0):
    sql = (
        f"SELECT ra,dec FROM {TABLE} WHERE brick_primary=1 AND "
        f"q3c_radial_query(ra,dec,{ra0:.8f},{dec0:.8f},{QUERY_RADIUS_DEG:.8f})"
    )
    df = pd.read_csv(io.StringIO(query(sql)))
    df.columns = [str(c).lower() for c in df.columns]
    if list(df.columns) != ['ra', 'dec']:
        raise RuntimeError(f'unexpected columns {list(df.columns)}')
    if len(df) == 0:
        return df.astype({'ra': float, 'dec': float}), sql
    half = BOX_DEG / 2
    dra = ((df.ra.astype(float) - ra0 + 180) % 360) - 180
...
def main() -> int:
    ...
    ap.add_argument('--out', default='data/real/dr11/expanded48')
    args = ap.parse_args()
    out = Path(args.out)
    ...
    for idx, (ra0, dec0) in enumerate(candidate_centers()):
        if len(accepted) >= TARGET_FIELDS:
            break
        if any(
            angular_sep(ra0, dec0, a['center_ra_deg'], a['center_dec_deg']) < MIN_SEP_DEG
            for a in accepted
        ):
            trials.append({'candidate_index': idx, 'ra': ra0, 'dec': dec0, 'status': 'prequery_separation_reject'})
            continue

        print(f'[DR11-48] candidate {idx}: ({ra0:.1f},{dec0:.1f})', flush=True)
        df, sql = fetch_candidate(ra0, dec0)
        n = len(df)
        if n < MIN_ROWS:
            trials.append({'candidate_index': idx, 'ra': ra0, 'dec': dec0, 'status': 'coverage_reject', 'rows': int(n)})
            print(f'[DR11-48] rejected coverage: {n:,}', flush=True)
            continue
        ...
        rec = {
            ...
            'query': sql,
            'rows': int(n),
            'canonical_csv_sha256': sha256(raw),
            'stored_gzip_sha256': sha256(gz),
            'file': str(path),
        }
        accepted.append(rec)
        trials.append({'candidate_index': idx, 'ra': ra0, 'dec': dec0, 'status': 'accepted', 'rows': int(n), 'name': name})
        print(f'[DR11-48] accepted {name}: {n:,}', flush=True)

    if len(accepted) != TARGET_FIELDS:
        raise RuntimeError(f'only {len(accepted)} acceptable fields; expected {TARGET_FIELDS}')
    ...
    prov = {
        ...
        'field_selection': 'fixed shuffled RA/Dec candidate grid; coverage threshold and pre-query angular-separation rule',
        'candidate_seed': 20260824,
        'target_fields': TARGET_FIELDS,
        'min_rows': MIN_ROWS,
        'min_separation_deg': MIN_SEP_DEG,
        'regions': accepted,
        'candidate_trials': trials,
        'total_rows': int(sum(r['rows'] for r in accepted)),
    }
    (out / 'provenance.json').write_text(json.dumps(prov, indent=2, sort_keys=True) + '\n')
    print(json.dumps({'status': 'REAL_DR11', 'fields': len(accepted), 'rows': prov['total_rows'], 'trials': len(trials)}, indent=2))
    return 0
