#!/usr/bin/env python3
"""Materialize the first 24 provenance-fixed REAL_DR11 expanded fields.

The repository already records the accepted centers, original q3c queries,
row counts and SHA-256 hashes.  This script re-runs only the first 24 indexed
...
Why 24?  The downstream selection-null analysis needs 12 bricks with adequate
pixel-level depth/NEXP/primary support.  It scans these pre-registered fields
in their original order and rejects fields *only* for insufficient observing
coverage, never for scientific outcome.
"""
...
SIDECAR=Path('data/real/dr11/expanded48/selection_bricks24.json')
N=24
...
def main():
    ...
    for i,r in enumerate(p['regions'][:N]):
        print(f"[materialize24] {i+1}/{N} {r['name']}",flush=True)
        ...
        brick=str(d.iloc[j].brickname).strip(); near=float(np.sqrt(tangent_sep2([d.iloc[j].ra],[d.iloc[j].dec],ra0,dec0)[0]))
        ...
        mappings.append({'name':r['name'],'center_ra_deg':ra0,'center_dec_deg':dec0,'brick':brick,
                         'nearest_primary_source_deg':near,'mapping_query':sql,'science_file':str(path),
                         'canonical_csv_sha256':r['canonical_csv_sha256'],'stored_gzip_sha256':r['stored_gzip_sha256']})
