#!/usr/bin/env python3
"""Fetch bounded, real DESI Legacy Surveys DR11 source catalogs.

This script intentionally has no simulation fallback. If DR11 cannot be reached,
it exits non-zero and no observational result is produced.
"""
...
# Independent sky fields. All are queried from the real DR11-South Tractor table.
# The analysis split is by whole field, not by neighbouring patches.
REGIONS = [
    ("cosmos", 150.12, 2.21),
    ("xmm_lss", 35.70, -4.75),
    ("cdfs", 53.10, -27.80),
...
def query_with_retries(sql: str, attempts: int = 4) -> str:
    ...
    for i in range(attempts):
        try:
            # Public synchronous queries do not require an account token.
            ...
            return result
        except Exception as exc:  # preserve failure; never synthesize data
            last = exc
            if i + 1 < attempts:
                time.sleep(8 * (i + 1))
...
""".strip()
    ...
    required = {"ra", "dec", "type", "maskbits"}
    missing = required - set(df.columns.str.lower())
    if missing:
        raise RuntimeError(f"{name}: required columns missing: {sorted(missing)}")
    df.columns = [c.lower() for c in df.columns]

    # Exact square used by the downstream gridding experiment.
    ...
    df = df.loc[keep, ["ra", "dec", "type", "maskbits"]].copy()
    df["type"] = df["type"].astype(str).str.strip()
    df = df.sort_values(["ra", "dec", "type", "maskbits"], kind="mergesort").reset_index(drop=True)
    ...
    return {
        ...
        "file": str(path),
        "type_counts": {str(k): int(v) for k, v in df["type"].value_counts().to_dict().items()},
        "maskbits_zero_rows": int((pd.to_numeric(df["maskbits"], errors="coerce").fillna(-1) == 0).sum()),
    }
...
def main() -> int:
    ...
    split = {
        "train": [r[0] for r in REGIONS[:8]],
        "validation": [r[0] for r in REGIONS[8:10]],
        "test": [r[0] for r in REGIONS[10:]],
    }
    provenance = {
        "status": "REAL_DR11",
        "dataset": DATASET,
        ...
        "analysis_tracers": {
            "all_primary": "all retrieved rows",
            "extended_clean": "maskbits == 0 and type != PSF",
        },
        "regions": region_meta,
        "field_split": split,
