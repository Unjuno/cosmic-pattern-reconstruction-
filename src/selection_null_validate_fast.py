...
Observed target: official DR11 Tractor BRICK_PRIMARY RA/Dec counts.
Selection controls: official DR11 depth-r, maskbits, nexp-r, psfsize-r maps.
Brick naming uses only a tiny Data Lab cone query; full source rows come from the
official NERSC Tractor FITS, avoiding large TAP/CSV transfers.
...
ROOT='https://portal.nersc.gov/cfs/cosmo/data/legacysurvey/dr11/south'
COADD=f'{ROOT}/coadd'; TRACTOR=f'{ROOT}/tractor'; TABLE='ls_dr11.tractor_s'
...
def get_bytes(url, attempts=5, timeout=120):
    last=None
    for i in range(attempts):
        try:
            r=requests.get(url,timeout=timeout); r.raise_for_status(); b=r.content
...
def read_image(url,wanted=None):
    ...
    with fits.open(io.BytesIO(b),memmap=False) as H:
        ...
        if not c: raise RuntimeError(f'no image HDU {url}')
...
def product_urls(brick):
    root=f'{COADD}/{brick[:3]}/{brick}'
    return {k:f'{root}/legacysurvey-{brick}-{s}.fits.fz' for k,s in {'depth_r':'depth-r','maskbits':'maskbits','nexp_r':'nexp-r','psfsize_r':'psfsize-r'}.items()}
...
def choose_brick_from_center(ra,dec):
    # LIMIT keeps this a tiny metadata-like query; it is not used for source density.
    for radius in [0.03,0.06,0.12]:
        sql=(f"SELECT brickname,ra,dec FROM {TABLE} WHERE brick_primary=1 "
             f"AND q3c_radial_query(ra,dec,{ra:.8f},{dec:.8f},{radius:.8f}) LIMIT 200")
        d=query_df(sql)
        if len(d):
...
def source_catalog(brick):
    url=f'{TRACTOR}/{brick[:3]}/tractor-{brick}.fits'
    b=get_bytes(url,timeout=180)
    with fits.open(io.BytesIO(b),memmap=False) as H:
        tab=next((h.data for h in H if h.data is not None and getattr(h.data,'dtype',None) is not None and h.data.dtype.names),None)
        if tab is None: raise RuntimeError(f'no table HDU in {url}')
        names={n.lower():n for n in tab.dtype.names}
        if 'ra' not in names or 'dec' not in names: raise RuntimeError(f'RA/DEC missing in {url}')
        keep=np.ones(len(tab),bool)
        if 'brick_primary' in names: keep=np.asarray(tab[names['brick_primary']]).astype(bool)
        ra=np.asarray(tab[names['ra']],float)[keep]; dec=np.asarray(tab[names['dec']],float)[keep]
    d=pd.DataFrame({'ra':ra,'dec':dec})
    return d,{'url':url,'sha256':sha256(b),'bytes':len(b),'rows':int(len(d))}
def sample_cells(a):
...
def continuous_features(a,log=False,positive_only=False):
    ...
    return [np.nan_to_num(np.nanmean(vals,axis=(2,3)),nan=0.0),np.nan_to_num(np.nanstd(vals,axis=(2,3)),nan=0.0),good.mean(axis=(2,3))]
...
def main():
    ...
    out=Path(args.out); out.mkdir(parents=True,exist_ok=True); centers=json.loads(Path(args.centers).read_text()); regs=centers.get('regions',[])[:N_FIELDS]
    ...
    for j,r in enumerate(regs):
        ...
        src,spv=source_catalog(brick); counts,ninside=count_grid(src,WCS(hdr),depth.shape)
        feat=[]; fn=[]
        ...
        provenance.append({'field':name,'brick':brick,'center_ra_deg':ra,'center_dec_deg':dec,'nearest_primary_source_deg':near,'brick_choice_query':bq,'source_product':spv,'source_rows':int(len(src)),'sources_inside_wcs':ninside,'valid_cell_fraction':float(valid.mean()),'n_patches':len(patches),'products':{'depth_r':dp,'maskbits':mp,'nexp_r':npv,'psfsize_r':pp}})
    ...
    prov={'status':'REAL_DR11_PIXEL_SELECTION_NULL','n_bricks':N_FIELDS,'brick_selection':'fixed sky center -> nearest BRICK_PRIMARY source in small cone; no density ranking','source_products':'official NERSC Tractor FITS','selection_products':['depth-r','maskbits','nexp-r','psfsize-r'],'regions':provenance,'total_source_rows':int(sum(x['source_rows'] for x in provenance))}; (out/'provenance.json').write_text(json.dumps(prov,indent=2,sort_keys=True)+'\n')
